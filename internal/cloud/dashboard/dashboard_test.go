...
func TestMountHTMXAndProjectDetailParity(t *testing.T) {
	...
	t.Run("stats and activity routes expose richer dashboard surfaces", func(t *testing.T) {
		...
		if !strings.Contains(stats.Body.String(), "Projects") {
...
func TestDashboardHomeHTMXWiring(t *testing.T) {
	...
	mux.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/dashboard/?auth=ok", nil))
	body := rec.Body.String()
	// The home now renders the Omnia command-center overview inline (mirroring the
	// local dashboard), not an HTMX lazy-load. Assert the bento panels are present.
	for _, marker := range []string{
		"Total Memories",
		"Memory Types",
		"Live Feed",
...
func TestDashboardHomeStatCardsAreClickable(t *testing.T) {
	...
	// Verify via HTML parse that at least one <a> element is a clickable stat card
	// (/dashboard/stats renders the Omnia bento via DashboardStatsPartial — panel-link tiles).
	...
	walkC6 = func(n *nethtml.Node) {
		if n.Type == nethtml.ElementNode && n.Data == "a" {
			// Check if <a> itself is a stat card (Omnia panel-link tile).
			for _, attr := range n.Attr {
				if attr.Key == "class" && strings.Contains(attr.Val, "panel-link") {
					foundLinkedCard = true
					return
				}
			}
			// Check if <a> wraps a stat card child.
	...
	walkC6(doc)
	if !foundLinkedCard {
		t.Errorf("C6: expected at least one stat card linked via <a> in /dashboard/stats body, got body=%q", body)
	}
}
