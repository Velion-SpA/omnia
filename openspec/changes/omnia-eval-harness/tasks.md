...
## STATUS: ALL 4 PRs COMPLETE — full slice done

PR1 (Phases 1-2, EVAL-1+EVAL-2) MERGED to main (#103). PR2 (Phases 3-4, EVAL-5+EVAL-3) MERGED (#105). PR3 (Phases 5-6, EVAL-4+EVAL-7) MERGED (#107). PR4 (Phases 7-9, EVAL-6+EVAL-8+integration/docs) implemented on branch `feat/eval-harness-cli-gate` — NOT yet committed (per orchestrator instruction: "Do NOT commit"). `go test ./internal/eval/... -race -cover`: PASS, 93.4% coverage. `go test ./cmd/omnia/...`: PASS. `go vet ./...` and `gofmt` clean on every touched file. `go build ./...` OK. cgo-free throughout. Full detail: engram `sdd/omnia-eval-harness/apply-progress`.
...
## Phase 7: Reproducibility Runner (EVAL-6) — COMPLETE (PR4)

- [x] 7.1 [RED] `internal/eval/runner_test.go`: `TestRunHarness_ExecutesThreeToFiveRuns`, `TestRunHarness_ReportsMeanAndStddev`, `TestGate_SingleRunRefusesDecision` (plus `TestRunHarness_RejectsOutOfBoundsRuns`, `TestRunHarness_PropagatesRunError`, `TestRunHarness_NilRunFuncErrors` edge cases). Also added `internal/eval/harness_test.go` (`RunOnce` dispatch tests, the retrieval-seam that `Config.Run` closures wrap) and `internal/eval/harness_integration_test.go` (Phase 9.1).
- [x] 7.2 [GREEN] `internal/eval/runner.go`: `RunFunc`, `Config{Run RunFunc}`, `MinRuns`/`MaxRuns` (3/5), `MetricStats{Mean,StdDev,N}`, `SegmentStats{Label,Accuracy,QualityPer1k}`, `RunSummary{Runs,ByCapability,ByLanguage,Overall,Reports}`, `RunHarness(ctx, cfg, runs) (RunSummary, error)` — validates `runs` in [3,5], executes `cfg.Run` exactly `runs` times, computes sample mean/stddev (N<2 → stddev 0) of accuracy and quality-per-1k-tokens per capability, per language, and overall (all-capabilities fold, since every case has exactly one capability). Also added `internal/eval/harness.go`: `RetrievedCase`, `RetrievedFetcher`, `RunOnce(ctx, cases, fetch, judge, relations) (Report, error)` — the single end-to-end pass `Config.Run` wraps, dispatching each case through `ScoreContradiction` (SupersedesOf set) or `Score` (spec EVAL-5) and folding into `BuildReport`. This extra file was not itemized in this task's original file list but is the natural single-pass seam `RunHarness` needed to be genuinely testable/reusable by both the CLI and harness_integration_test.go — added per architecture-guardrails ("real behavior belongs in Go packages", CLI stays thin).

## Phase 8: CLI + Release-Gate Wiring (EVAL-8) — COMPLETE (PR4)

- [x] 8.1 [RED] `cmd/omnia/eval_test.go`: `TestCmdEval_AdvisoryNeverBlocks`, `TestCmdEval_BlockingExitsNonZeroPastThreshold` (plus `TestCmdEval_BlockingWithinThresholdDoesNotExit`, `TestCmdEval_NoBaselineSkipsGate`, `TestCmdEval_InvalidModeExitsNonZero`, `TestCmdEval_HarnessErrorExitsNonZero`). Also added `internal/eval/gate_test.go` for the pure gate-decision logic (`TestGate_AdvisoryNeverBlocks`, `TestGate_BlockingFailsPastThreshold`, `TestGate_BlockingWithinThresholdDoesNotBlock`, `TestGate_InsufficientRunsRefusesDecision`, `TestGate_InvalidModeErrors`).
- [x] 8.2 [GREEN] `cmd/omnia/eval.go`: `cmdEval(args []string)` following `cmdEmbed`'s `flag.NewFlagSet` pattern; flags `--mode advisory|blocking` (default `advisory`), `--runs` (default 3), `--threshold` (default 0.05), `--baseline` (default 0, skips the gate), `--corpus`, `--ab-pairs`, `--config`; `case "eval": cmdEval(os.Args[2:])` added to the switch in `cmd/omnia/main.go` (plus a `printUsage()` entry). `defaultRunEvalHarness` wires the real corpus (`eval.LoadCorpus`, enforcing spec EVAL-2's [50,150] floor), the real store (`storeNew`) via a `storeBackedFetcher` (FTS search per case), an optional LLM judge (`llm.NewRunner` when `OMNIA_AGENT_CLI` is set — required by spec EVAL-5 for causal/state_abstraction cases, absence surfaces as a real per-case error, not a silent miss), and a best-effort retrieval-only AB-pairs section (spec EVAL-7, skipped gracefully when embeddings are disabled/unreachable). Also added `internal/eval/gate.go`: `GateMode`, `GateResult`, `EvaluateGate(summary, mode, baselineAccuracy, threshold) (GateResult, error)` — the pure EVAL-8 decision logic `cmdEval` calls (defense-in-depth: refuses summaries with `Runs < MinRuns` even if hand-built, per spec EVAL-6).
- [x] 8.3 Wired `.goreleaser.yaml` `before.hooks`: added a documented, COMMENTED-OUT hook (not active) showing the exact `omnia eval --mode advisory ...` invocation to uncomment once the corpus reaches its 50-case floor and a real `--baseline` has been established — kept inert per the orchestrator's explicit "do NOT break the existing goreleaser release flow... be conservative" instruction, since a live hook today would abort every release (corpus is 11/50 cases). Verified `goreleaser check` still reports the config valid (pre-existing `brews` deprecation warning only, unrelated to this change).

## Phase 9: Integration + Docs — COMPLETE (PR4)

- [x] 9.1 `internal/eval/harness_integration_test.go`: `TestHarnessIntegration_EndToEndAcrossAllSections` — end-to-end run against the REAL `testdata/cases.json` (11 dogfooded cases, loaded via package-internal `parseCorpus` to exercise real wiring today since the file is intentionally below `LoadCorpus`'s 50-case production floor — see PR1 apply-progress) + the REAL `internal/embed/testdata/ab_pairs.json`, through `RunOnce` → `RunHarness` (3 runs) against a self-contained temp `*store.Store` (same bootstrap as `contradiction_test.go`) and a fake LLM judge (hermetic, no live CLI). Asserts every report section is present: per-capability + per-language segments (EVAL-3), a distinct non-nil retrieval-only section per run (EVAL-7), and mean+stddev (N=3) for accuracy and quality-per-1k-tokens on every capability/language/overall segment (EVAL-6). 93.4% package coverage, `-race` clean.
- [x] 9.2 Documented `omnia eval` usage and gate config in `DOCS.md` (new "Eval Harness CLI (admin)" section, alongside the existing "Conflict Audit CLI (admin)" section) — flags, the corpus floor caveat, the retrieval-section best-effort behavior, and the advisory/blocking gate semantics. Smoke-tested the real binary end-to-end (both a below-floor-corpus refusal and a synthetic 50-case corpus full run) to verify every documented behavior actually executes as described (docs-alignment "Command examples execute as documented").
