...
- **PR2 (reconcile/read side) — NOT STARTED**: Phase 4 (`ScanProject{Source:anchor}` travel/staleness), Phase 5 (`omnia forget-scan` CLI), Phase 6 (recall stale downrank + receipt), Phase 7 (flag/regression/docs).
...
- [ ] 6.1 [TEST] `internal/mcp/anchor_adapter_test.go`: penalty function reduces score deterministically (pure; `internal/recall.Fuse` untouched)
- [ ] 6.2 `anchor_adapter.go`: downrank fn + `BuildStaleReceipt` -> "anchor <file>:<lines> changed <old->new sha>"
- [ ] 6.3 [TEST] `mem_search`: stale-anchored memory ranks below equally-relevant fresh one; receipt line present
- [ ] 6.4 `mcp.go` `handleSearch`: after `HydrateFusedResults` (~line 1089) and the plain `s.Search` branch, batch-load anchor status, re-sort, append receipt to text+structured entry
- [ ] 6.5 [TEST] memory with no anchor unaffected (regression)

## Phase 7: Regression / flag / docs

- [ ] 7.1 `internal/config/config.go`: add `structural_forgetting.enabled` (default true) gating only recall downrank (capture always runs if `code_anchors` supplied)
- [ ] 7.2 Full `go test ./...` — all Phase 3/4 conflict tests GREEN unchanged
- [ ] 7.3 `CGO_ENABLED=0 go build ./...` GREEN (cgo-free verdict)
