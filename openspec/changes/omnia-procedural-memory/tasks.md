...
- [ ] 4.1 [RED] `internal/mcp/procedure_induction_test.go` (new): `outcome=worked` + fake inducer → `candidate` `playbook`, `source_obs_sync_ids` linked
- [ ] 4.2 `internal/mcp/mcp.go` `MCPConfig` (L40-80): add `Procedural *ProceduralWiring` field (mirror `AutoEmbed *embed.Worker`)
- [ ] 4.3 `mcp.go` `handleSave` (~L1447, after `Outcome` persisted): async best-effort induce call, log-and-swallow on error (mirror `FindCandidates` L1508-1512 / `enqueueAutoEmbed` non-blocking) — GREEN 4.1
- [ ] 4.4 [RED] `procedure_induction_test.go`: no `Procedural`/no runner configured → save succeeds, no procedure, no error
- [ ] 4.5 `cmd/omnia/main.go`: build `Procedural` wiring from `config.ProceduralConfig` when `enabled=true` — GREEN 4.4

## Phase 5: SSGM Reuse Attribution (spec: reuse-attribution)

- [ ] 5.1 [RED] `procedure_induction_test.go`: `applied_procedure` + `outcome=worked` → `ConfirmReuse` invoked, counters update
- [ ] 5.2 `mcp.go`: add `applied_procedure` arg to `mem_save`/`mem_update` schemas (~L417-421/458-459) + `handleSave`/`handleUpdate` wiring to `ConfirmReuse`/`Contradict` — GREEN 5.1
- [ ] 5.3 [RED] `procedure_induction_test.go`: no `applied_procedure`, no session match → safe no-op, save unchanged
- [ ] 5.4 `internal/mcp` (new helper): best-effort same-session `trigger`-vs-`error_signature` fallback match (mirror `resolveFallbackSessionID`) — GREEN 5.3

## Phase 6: Offline Batch Induction (spec: induction, `ScanProject` seam)

- [ ] 6.1 [RED] `internal/store/scan_procedure_test.go` (new): fake `ProcedureInducer`; 3 `worked` obs sharing `error_signature` + 2 unrelated `did_not_work` → `--apply` yields exactly 2 candidates
- [ ] 6.2 `internal/store/relations.go` or `procedures.go`: `InduceProject(opts InduceOptions) (InduceResult, error)` — cluster via `NormalizeErrorSignature`, reuse `ScanProject`'s worker-pool shape (L1340-1428 pairCh/wg/mu) — GREEN 6.1
- [ ] 6.3 [RED] `scan_procedure_test.go`: no `--apply` → counts only, zero rows written
- [ ] 6.4 `cmd/omnia/procedure.go` (new): `omnia procedure-induct [--project][--apply]` (mirror `cmdConflictsScan`, `conflicts.go` L302-476) — GREEN 6.3
- [ ] 6.5 `cmd/omnia/procedure.go`: `procedure list/inspect/retire` subcommands (mirror `cmdConflictsList`/`Show`)
- [ ] 6.6 `cmd/omnia/procedure_test.go` (new): flag parsing, `--apply` vs dry-run, no-LLM graceful skip (mirror `conflicts_test.go` `stubAgentRunnerFactory`)
- [ ] 6.7 `cmd/omnia/main.go` (~L725): register `"procedure-induct"`/`"procedure"` dispatch case

## Phase 7: Contrastive Retrieval (spec: contrastive-retrieval)

- [ ] 7.1 [RED] `internal/mcp/procedure_card_test.go` (new): matching query → one card, top trusted playbook + top trusted anti-playbook, confidence values
- [ ] 7.2 [RED] `procedure_card_test.go`: only one polarity trusted → card has that polarity only, no fabricated pair
- [ ] 7.3 `internal/mcp/recall_adapter.go`: `BuildProcedureCard(s, query, project, scope)` — picks top trusted each polarity via `SearchProcedures` (mirror `HydrateFusedResults`; `recall.Fuse` untouched/pure) — GREEN 7.1-7.2
- [ ] 7.4 `mcp.go` `handleSearch` (~L1019) + `handleContext` (~L1810): attach card to envelope when `cfg.Procedural` enabled (mirror `extra["outcome"]` L1167)

## Phase 8: Backward-Compat, Sync Exclusion, Docs

- [ ] 8.1 [RED] `mcp_test.go`: `procedural.enabled=false` + `outcome=worked` → response identical to pre-feature baseline
- [ ] 8.2 [RED] `internal/store/store_test.go`: cloud sync payload builder never emits `procedures` rows (no `SyncEntityProcedure` const, no `enqueueSyncMutationTx` call from `procedures.go`)
- [ ] 8.3 `docs/PROCEDURAL_MEMORY.md` (new): schema, polarity, governance lifecycle, retrieval card, forward-compat note to deferred compiler (#1592)
- [ ] 8.4 Full regression: `CGO_ENABLED=0 go test ./...` GREEN, incl. signature-lane + conflict-semantic suites unchanged
