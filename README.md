...
| `recall.ranking.weights.salience` | `0` | Multiplier for a machine-written salience signal (separate from human-curated `importance`, never auto-filled like its siblings — opt-in only). Has no effect while `learned_ranker.enabled` is on with a trained model loaded — the learned ranker's fixed feature set has no salience input |
...
| `injection.budget.max_tokens` | `400` | Estimated-token ceiling for `injection.budget` (v0.3.1 recalibration, eval-backed — explicitly configured values, including the old `1500`, are preserved as-is); the topic_key exact-match sentinel and error-signature-match rows are always emitted complete and never count against this budget |
...
| `OMNIA_SIGNAL_RECALL` (env var) | unset (off) | Hook-layer gate for the signal-gated recall nudge in the Claude Code plugin's `user-prompt-submit.sh`: on new-topic/uncertainty signals (EN+ES) it instructs the agent to run `mem_search` with extracted keywords — an instruction-only nudge, never auto-injected results. This is the one v0.3 feature gated by an env var instead of `config.yaml` (bash hooks cannot parse YAML); unset/`0` is byte-for-byte identical to the pre-v0.3 hook |
| `OMNIA_SIGNAL_RECALL_COOLDOWN_SECS` (env var) | `300` | Minimum seconds between signal-recall nudges in one session (on top of per-topic dedup markers), so chatty Q&A sessions are never nudged every turn |
| `write_hygiene.enabled` | `true` | v0.3.1 save-side write-gate (NOOP / auto-update / save+relate on every save). The ONE default-ON gate: it changes only what gets STORED, never what an agent receives; an explicit `enabled: false` is the kill-switch and restores byte-for-byte pre-v0.3.1 save behavior |
| `write_hygiene.noop_threshold` | `0.98` | Token-set Jaccard similarity at-or-above which an incoming save is treated as content-identical: nothing is stored and the existing observation's ID is returned |
