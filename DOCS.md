...
```
omnia dedupe [--project <name>] [--all-projects] [--json] [--dry-run]
omnia dedupe --apply <cluster-id> [--project <name>] [--all-projects] [--json]
```
...
- `--all-projects` (optional, cannot be combined with `--project`): widens candidate matching itself across every project, not just each observation's own. **This is a real, source-verified gap the plain (no-flag) default does NOT close**: with no `--project` at all, the scan already *lists* observations from every project, but each observation's candidate lookup historically stayed scoped to its own project — so the exact same content saved under two different `project` keys (e.g. the same GitHub PR ingested twice under two project names) never clustered. `--all-projects` fixes candidate generation itself to cross that boundary; cluster members are always annotated with their own `project` in the report (human and `--json`), regardless of which mode found them.
...
Candidate generation reuses the same FTS-blocked pre-filter shape as the write-gate/`ScanProject` (FTS5 `MATCH` on title, same scope, bounded per-observation candidate cap, and — unless `--all-projects` widens it — the same project) — this is what keeps the scan `O(n * candidate_limit)`, never an all-pairs `O(n^2)` comparison, at real-corpus scale (1600+ observations completes in a few seconds). Candidates are then scored by the same content-Jaccard primitive the write-gate uses (`internal/similarity`); pairs scoring `>= 0.9` are unioned via union-find into clusters. Only same-type pairs are ever clustered together — a high-similarity match against a DIFFERENT observation type is treated as a related-but-distinct fact, not a duplicate (mirrors the write-gate's own type-gating precedent). Soft-deleted observations are never scanned or surfaced as candidates.

**`--apply` on a cross-project cluster REFUSES, cleanly, with no mutation.** A cluster discovered via `--all-projects` whose members span more than one distinct project is a conservative product boundary, not a bug: merging across projects changes ownership semantics (which project "owns" the surviving canonical row) that this release deliberately leaves unsupported. `--apply <cluster-id>` on such a cluster prints a clear refusal ("cross-project merges change ownership; not supported") and exits non-zero before touching anything; a same-project cluster found via `--all-projects` (the widened scan is a strict superset of the default one) still applies normally.
...
- **salience**: the machine-written salience signal (`Observation.Salience`, separate from human-curated `importance`), normalized into `[0,1]` and clamped defensively — `0` when the observation has no salience recorded. `recall.ranking.weights.salience` defaults to `0`, so this term does not move `final` unless an operator opts in. Has no effect at all when `learned_ranker.enabled: true` with a trained model loaded — the learned ranker's ordering replaces `final` outright and its fixed feature set has no salience input
- **final**: the weighted-sum ranking score (`recall.ranking.weights.*`) computed from the four components above — reflects what `recall.ranking.enabled: true` would sort by, even when ranking is currently off
...
- **code_anchors**: optional list of code anchors to link this memory to a location in the repo: `[{file, symbol, line_start, line_end}]` (`file`, `line_start`, `line_end` required; `symbol` optional). Each entry is resolved (explicit-anchor v1: file + symbol + git-blame line range, blamed via the local `git` binary — no cgo, no go-git) into a git-blame line range, blame SHA, and content hash, and persisted linked to this memory (living-memory / structural-forgetting). Omitting this field leaves `mem_save` unchanged. Anchoring degrades silently: a missing `git` binary, a directory that is not a git repository, or a malformed entry never fails the save — that entry (or all of them) is simply skipped.
...
The write-gate's internal candidate-detection BM25 floor is intentionally permissive (not user-configurable — a hardcoded internal threshold, not a `write_hygiene.*` field), so `noop`/`update` keep firing as the store grows. An earlier, tighter floor passed every small-fixture test but went silently inert on a real multi-thousand-observation base — BM25's IDF component grows with corpus size, so a genuinely rare (but real) title term's score can drift far more negative than any small-corpus test ever exercises, excluding every candidate and making `decision` degrade to plain `save` for content that should have NOOP'd or auto-updated. The fix mirrors `omnia dedupe`'s own equally permissive candidate floor, which independently hit and fixed the identical defect for the offline scan path first.
